...
constexpr T sign(T value) noexcept {
	if constexpr (std::floating_point<T>) {
		...
		else if (value) { return value < T{0} ? T{-1} : T{1}; }
		return T{0};
	}
	else if constexpr (std::signed_integral<T>) {
		if (value) { return value < T{0} ? T{-1} : T{1}; }
		return T{0};
	}
	else { return T{value != T{0}}; }
}

constexpr f32 floor(f32 value) noexcept {
	if (value != value || abs(value) >= DECIMAL_LIMIT_F) { return value; }
	f32 const truncated = static_cast<i32>(value);
