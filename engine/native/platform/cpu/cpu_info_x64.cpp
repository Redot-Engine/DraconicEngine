...
CpuFeature detect_cpu_feature() noexcept {
	...
	if (!(ebx & AVX2)) { return CpuFeature::NONE; }

	if ((ebx & AVX512F) && os_has_zmm()) { return CpuFeature::AVX512F; }
...
#if defined(__x86_64__) || defined(_M_X64)
	CpuFeature const level = detect_cpu_feature();

	if (level == CpuFeature::NONE) { std::abort(); }
#else
