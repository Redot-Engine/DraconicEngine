...
void queue_destruction(std::function<void()> cb) {
	g_deletion_queue.push_back({bgfx::getStats()->gpuFrameNum, std::move(cb)});
}

// Explicit overloads for each bgfx resource
void destroy_later(bgfx::ShaderHandle handle) {
	queue_destruction([handle]() { bgfx::destroy(handle); });
}
...
void process_deletions() {
	...
	std::erase_if(g_deletion_queue, [frame](auto const &d) {
		if (frame >= d.frame + 2) {
			d.cleanup();
			return true;
		}
...
void destroy_buffer(BufferHandle h) {
	...
	if (bgfx::isValid(buf->vbh)) { destroy_later(buf->vbh); }

	if (bgfx::isValid(buf->ibh)) { destroy_later(buf->ibh); }

	if (bgfx::isValid(buf->dvbh)) { destroy_later(buf->dvbh); }

	if (bgfx::isValid(buf->dibh)) { destroy_later(buf->dibh); }
